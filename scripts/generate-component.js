...
if (!name) {
  ...
  console.log('Usage: node scripts/generate-component.js <ComponentName> [--shared | --feature <featureName>]');
  process.exit(1);
}
...
if (isShared) {
  ...
  targetDir = path.join(process.cwd(), 'src', 'features', featureName, 'components');
} else {
...
if (fs.existsSync(targetFile)) {
  console.log('\x1b[33m%s\x1b[0m', `Warning: File already exists at ${targetFile}`);
  process.exit(1);
}
